import cholesky_window
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np
from CustomDialog import CustomDialog
from MatrixAnswer import MatrixAnswer
from Methods.Cholesky import CholeskyMethod


class Cholesky(QtWidgets.QWidget, cholesky_window.Ui_cholesky):
    def __init__(self, parent=None):
        super(Cholesky, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if self.a_matrix is not None and self.b_vector is not None:
            cholesky = CholeskyMethod(self.a_matrix, self.b_vector)
            cholesky.cholesky()
            answer = cholesky.answer
