import jacobi_window
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np
from Methods.Jacobi import JacobiMethod
from CustomDialog import CustomDialog
from MatrixAnswer import MatrixAnswer



class Jacobi(QtWidgets.QWidget, jacobi_window.Ui_jacobi):
    def __init__(self, parent=None):
        super(Jacobi, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if (self.a_matrix is not None) and (self.b_vector is not None):
            jac = JacobiMethod(self.a_matrix,self.b_vector, self.x0_vector, self.t, self.iter)
            jac.jacobi()
            answer = jac.answer
