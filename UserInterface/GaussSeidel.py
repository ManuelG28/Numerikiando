import gauss_seidel_window
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np
from Methods.GaussSeidel import GaussSeidelMethod
from CustomDialog import CustomDialog
from MatrixAnswer import MatrixAnswer



class GaussSeidel(QtWidgets.QWidget, gauss_seidel_window.Ui_gauss_seidel):
    def __init__(self, parent=None):
        super(GaussSeidel, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if (self.a_matrix is not None) and (self.b_vector is not None):
            gs = GaussSeidelMethod(self.a_matrix,self.b_vector, self.x0_vector, self.t, self.iter)
            gs.gaussSeidel()
            answer = gs.answer
