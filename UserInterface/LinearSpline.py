...
class LinearSpline(QtWidgets.QWidget, linear_spline_window.Ui_linear_spline):
    def __init__(self, parent=None):
        super(LinearSpline, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if (self.x1_vector is not None):
            a = self.linearSpline(self.x1_vector)
            print(a)
    ...
    def linearSpline(self, x):
        matrix=np.zeros(shape=(2*(len(x)-1),2*(len(x)-1)))
        countPair=0
