...
class SquareSpline(QtWidgets.QWidget, square_spline_window.Ui_square_spline):
    def __init__(self, parent=None):
        super(SquareSpline, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if (self.x1_vector is not None):
            a = self.squareSpline(self.x1_vector)
            print(a)
    ...
    def squareSpline(self,x):
        matrix=np.zeros(shape=(3*(len(x)-1),3*(len(x)-1)))
        countPair=0
