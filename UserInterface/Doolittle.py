import doolittle_window
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np
from CustomDialog import CustomDialog
from MatrixAnswer import MatrixAnswer
from Methods.Doolittle import DoolittleMethod


class Doolittle(QtWidgets.QWidget, doolittle_window.Ui_doolittle):
    def __init__(self, parent=None):
        super(Doolittle, self).__init__(parent)
        self.setupUi(self)
    ...
    def runMethod(self):
        ...
        if self.a_matrix is not None and self.b_vector is not None:
            doolittle = DoolittleMethod(self.a_matrix, self.b_vector,self.n)
            doolittle.doolittle()
            answer = doolittle.answer
